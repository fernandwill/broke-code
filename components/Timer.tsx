...
const formatTime = (time: number): string => {
    ...
    return <div>
        {showTimer ? (
            <div className="flex items-center space-x-2 bg-[#FFFFFF24] p-1.5 cursor-pointer rounded hover:bg-[#FFFFFF1A]">
                <div>{formatTime(time)}</div>
                <FiRefreshCcw onClick={() => setShowTimer(false)} />
            </div>
        ) : (
            <div className="flex items-center p-1 h-8 hover:bg-[#FFFFFF1A] rounded cursor-pointer" onClick={() => {setShowTimer(true);  setTime(0)}}>
                <svg
